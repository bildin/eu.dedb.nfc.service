...
public class NfcService extends Service {
    ...
    private BroadcastReceiver mBroadcastReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (UsbManager.ACTION_USB_DEVICE_ATTACHED.equals(action)) {
                ...
                UsbDevice device = intent.getParcelableExtra(UsbManager.EXTRA_DEVICE);
                PCDBinder pcdBinder = binders.remove(device);
                if (pcdBinder != null) {
                    pcdBinder.stopPolling();

                    if (binders.size() == 0) {
                        mNotification.update(NfcService.SERVICE_NOTIFICATION_ID, Notification.STATE_IDLE);
                    }
                }
            } else if (NfcService.ACTION_STOP_SERVICE.equals(action)) {
                stopSelf();
            } else if (Intent.ACTION_SCREEN_ON.equals(action)) {
                isPaused = false;
            } else if (Intent.ACTION_SCREEN_OFF.equals(action)) {
                boolean settings_ignore_screen_lock_state = preferences.getBoolean(
                        Settings.PREFERENCES_settings_ignore_screen_lock, false);
    ...
    public void onCreate() {
        ...
        // Get Preferences
        preferences = getSharedPreferences(Settings.PREFERENCES, Context.MODE_PRIVATE);
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        } else if (NfcService.ACTION_USB_PERMISSION.equals(action)) {
            if (intent.getBooleanExtra(UsbManager.EXTRA_PERMISSION_GRANTED, false)) {
                UsbDevice device = intent.getParcelableExtra(UsbManager.EXTRA_DEVICE);
                if (!binders.containsKey(device)) {
                    PCD pcd = null;
                    // let do 3 tries to connect
                    for (int attempt = 0; attempt < 2; attempt++) {
                        pcd = getPCD(device);
                        if (pcd != null) {
                            mNotification.update(NfcService.SERVICE_NOTIFICATION_ID, pcd.toString());

                            PCDBinder pcdBinder = new PCDBinder(pcd);
                            binders.put(device, pcdBinder);
                            pcdBinder.startPolling(NfcService.POLLING_INTERVAL);
                            break;
                        }
                    }
    ...
    private PCD getPCD(UsbDevice mUsbDevice) {
        ...
        // Get serial settings
        int baudrate = preferences.getInt(Settings.PREFERENCES_settings_baudrate, 0);

        // Try MFRC522 Readers
        MFRC522_UART mMFRC522 = MFRC522_UART.get(getApplicationContext(), mUsbDevice, baudrate);
        if (mMFRC522 != null) {
        ...
        // Try PN53x Readers
        PN53X_UART mPN53X = PN53X_UART.get(getApplicationContext(), mUsbDevice, baudrate);
        if (mPN53X != null) {
    ...
    class PCDBinder extends Binder {
                    ...
                    synchronized (mPCD) {
                        ...
                        if (mPICC != null) {
                            ...
                            if (AppChooser.isActive()) {
                                mNotification.update(NfcService.SERVICE_NOTIFICATION_ID,
        ...
        public PCDBinder(PCD pcd) {
            ...
            this.attachInterface(null, NfcService.DESCRIPTOR);
        ...
        private boolean recoverPCD() {
            ...
            if (mPCD != null) {
                ...
                mNotification.update(NfcService.SERVICE_NOTIFICATION_ID, Notification.STATE_ERROR);
            }
            if (mPCD == null) {
                stopPolling();
                return false;
        ...
        protected boolean onTransact(int code, Parcel data, Parcel reply, int flags) throws RemoteException {
            ...
            synchronized (mPCD) {
                if (tName == null) {
                    ...
                    data.enforceInterface(NfcService.DESCRIPTOR);
                    ...
                    if (mPICC != null) {
                        try {
                            // TODO check pseudo commands
                            if (!raw && sendBuf[0] == (byte) 0xFF) { // Pseudo command Control
                                pausePolling(sendBuf[1] != 0);
                                PICC polled = mPCD.poll(0);
                                if (polled != null) {
                                    tresponse = new TransceiveResponse(TransceiveResponse.RESULT_SUCCESS, polled.getTag(this).getId(), 0);
                                } else {
                                    tresponse = TransceiveResponse.getIOErrorResponse();
                                }
                            } else if (!raw && sendBuf[0] == 0x00) { // Pseudo command Transceive
                                int bitFrarming = sendBuf[1] & 0xFF;
                                int pcdTimeout = (sendBuf[2] & 0xFF) * 10;
                                int pcdFlags = sendBuf[3] & 0xFF;
                                byte[] data2 = new byte[sendBuf.length - 4];
                                System.arraycopy(sendBuf, 4, data2, 0, data2.length);
                                tresponse = mPCD.transceive(data2, bitFrarming, pcdTimeout, pcdFlags);
                            } else {
                    ...
                    // seems to be useless (doubles tresponse.writeToParcel call)
                    /*
        ...
        private void dispatchTag(Tag tag) {
            ...
            if (!system_dispatched) {
                // Create targetIntent
                Intent targetIntent = new Intent(NfcAdapter.ACTION_TECH_DISCOVERED);
                ...
                targetIntent.putExtra(NfcService.EXTERNAL_NFC_SERVICE,
